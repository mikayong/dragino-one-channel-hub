...
uint32_t lgw_radio_timestamp_correction( uint32_t sf, uint8_t bw )
{
    ...
    case BW_500KHZ:
        ...
        break;
    default:
        ESP_LOGE( TAG_HAL_RX, "ERROR: unsupported bandwidth 0x%02X (%s)", bw, __FUNCTION__ );
    }

    if( bw_pow != 0 )
    {
        uint32_t t_symbol_us = ( 1 << sf ) * 8 / bw_pow; /* 2^SF / BW , in microseconds */
        return t_symbol_us;
    }
#elif defined( CONFIG_RADIO_TYPE_LR1121 )
    ESP_LOGW( TAG_HAL_RX, "no timestamp correction" );
#else
